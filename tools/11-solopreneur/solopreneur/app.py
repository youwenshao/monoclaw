"""Unified FastAPI application for the Solopreneur Dashboard."""
...
from solopreneur.database import init_all_databases
...
logger = setup_logging("solopreneur")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("Solopreneur Dashboard starting on port %d", config.port)
    logger.info("Databases initialized: %s", list(db_paths.keys()))
    yield
    logger.info("Solopreneur Dashboard shutting down")


app = FastAPI(
    title="Solopreneur Dashboard",
    version="1.0.0",
    lifespan=lifespan,
)
...
def _ctx(request: Request, **extra: object) -> dict:
    ...
    return {
        ...
        "active_tab": extra.get("active_tab", "biz-owner-os"),
        **extra,
    }
...
async def index(request: Request) -> HTMLResponse:
    return RedirectResponse("/biz-owner-os/")  # type: ignore[return-value]
...
async def save_setup(request: Request) -> RedirectResponse:
    ...
    from solopreneur.seed_data import seed_all

    form = await request.form()
    config = request.app.state.config

    config.extra["business_name"] = form.get("business_name", "")
    config.extra["br_number"] = form.get("br_number", "")
    config.extra["business_type"] = form.get("business_type", "")
    ...
    config.extra["pos_provider"] = form.get("pos_provider", "")
    config.extra["pos_api_key"] = form.get("pos_api_key", "")
    config.extra["mpf_trustee"] = form.get("mpf_trustee", "")
    config.extra["mpf_scheme_name"] = form.get("mpf_scheme_name", "")
    config.extra["instagram_access_token"] = form.get("instagram_access_token", "")
    config.extra["facebook_page_id"] = form.get("facebook_page_id", "")
    config.extra["facebook_access_token"] = form.get("facebook_access_token", "")
    config.extra["primary_bank"] = form.get("primary_bank", "")
...
async def connection_test(request: Request) -> HTMLResponse:
    ...
    results.append({
        "name": "POS System",
        "ok": bool(config.extra.get("pos_api_key")),
    })
    results.append({
    ...
    results.append({
        ...
        "ok": bool(config.extra.get("facebook_access_token")),
    })
...
from solopreneur.biz_owner_os.routes import router as biz_owner_os_router  # noqa: E402
from solopreneur.mpf_calc.routes import router as mpf_calc_router  # noqa: E402
from solopreneur.supplier_ledger.routes import router as supplier_ledger_router  # noqa: E402
from solopreneur.social_sync.routes import router as social_sync_router  # noqa: E402

app.include_router(biz_owner_os_router)
app.include_router(mpf_calc_router)
app.include_router(supplier_ledger_router)
app.include_router(social_sync_router)

config = load_config(CONFIG_PATH)
_db_paths_for_health = {
    "bizowner": str(Path(config.database.workspace_path).expanduser() / "bizowner.db"),
    "mpf": str(Path(config.database.workspace_path).expanduser() / "mpf.db"),
    "ledger": str(Path(config.database.workspace_path).expanduser() / "ledger.db"),
    "socialsync": str(Path(config.database.workspace_path).expanduser() / "socialsync.db"),
}
app.include_router(create_health_router("solopreneur", "1.0.0", _db_paths_for_health))
app.include_router(
